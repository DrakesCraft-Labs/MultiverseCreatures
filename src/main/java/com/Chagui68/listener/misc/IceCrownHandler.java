...
public class IceCrownHandler implements Listener {
    ...
    private double icePathFreezeChance;
    ...
    public IceCrownHandler(Plugin plugin) {
        this.plugin = plugin;
        snowBlockCooldownMs = plugin.getConfig().getLong("items.ice-crown.snow-block-launch.cooldown-ms", 10000);
        blizzardCooldownMs = plugin.getConfig().getLong("items.ice-crown.blizzard.cooldown-ms", 60000);
        projectileSpeed = plugin.getConfig().getDouble("items.ice-crown.snow-block-launch.projectile-speed", 2.0);
        projectileMaxTicks = plugin.getConfig().getInt("items.ice-crown.snow-block-launch.max-ticks", 100);
        blizzardDamagePerTick = plugin.getConfig().getDouble("items.ice-crown.blizzard.damage-per-tick", 3.5);
        icePathFreezeChance = plugin.getConfig().getDouble("items.ice-crown.ice-path.adjacent-freeze-chance", 0.3);
    }
    ...
    public void onPlayerMove(PlayerMoveEvent e) {
        ...
        for (int dx = -1; dx <= 1; dx++) {
            for (int dz = -1; dz <= 1; dz++) {
                ...
                if (adj.getType() == Material.WATER && random.nextDouble() < icePathFreezeChance) {
